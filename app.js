...
(function() {
  ...
  fetchCommandForNumber = function(token, num, res) {
    return mongoClient.connect(mongoUri, function(err, db) {
      var collection, doc;
      if (err) {
        throw err;
      ...
      return doc = collection.findOne({
        ...
        if (item === null) {
          ...
          response = "";
          responseobjs = [];
          return docs.each(err, doc)(function() {
            if (err) {
              throw err;
            }
            if (doc === null) {
  ...
  server = http.createServer(function(req, res) {
    var basename, dirname, isIgnore, mail, number, params, password, pathname, query, token, user;
    isIgnore = false;
    ...
    if (basename.indexOf("postCommand") === 0) {
      ...
      return fetchCommandForNumber(token, number, res);
    } else {
