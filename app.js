...
(function() {
  var AUTH_COLLECTION, Command, DATA_COLLECTION, DOCROOT, S, USER_COLLECTION, addAuthenticate, authenticate, fs, getHandler, http, listCommands, log, loginAs, loginOrRegister, makeHTMLResponse, mongo, mongoClient, mongoUri, port, postCommand, querystring, server, socketio, url, uuid, writeAsHtml;
  ...
  loginAs = function(mail, password, res) {
  ...
  loginOrRegister = function(mail, password, res) {
    mongoClient.connect(mongoUri, function(err, db) {
      var collection, createdNewUser, docs, uid;
      if (err) {
        throw err;
  ...
  server = http.createServer(function(req, res) {
    var filepath, isIgnore, mail, params, password, pathname, query;
    filepath = '';
    isIgnore = false;
    ...
    if (pathname === '/') {
      ...
      return loginOrRegister(mail, password, res);
    } else if (pathname === "/loginAs") {
      query = url.parse(req.url).query;
      params = querystring.parse(query);
      mail = params.mail;
      password = params.password;
      return loginAs(mail, password, res);
