...
(function() {
  var AUTH_COLLECTION, Command, DATA_COLLECTION, DOCROOT, Engine, S, SESSION_COLLECTION, Session, USER_COLLECTION, User, authenticate, cleanupSession, cleanupSessionWithoutClosing, fetchCommandFromNumber, fs, getHandler, getParams, http, listCommands, log, loginAs, makeHTMLResponse, mineTypes, mongo, mongoClient, mongoUri, path, port, postCommand, querystring, registerToken, server, url, uuid, writeAsHtml;
  ...
  cleanupSessionWithoutClosing = function(db, collection, token) {
    return collection.remove({
      token: token
    }, function(err, item) {
      if (err) {
  ...
  Session = (function() {
    ...
    Session.prototype.SECOND = 1000;

    Session.prototype.MINUTE = 60 * Session.SECOND;

    Session.prototype.HOUR = 60 * Session.MINUTE;
